...
{"keep": true|false,
 ...
 "apply": "Достань из поста КОНКРЕТИКУ (техника/изменение/цифра/инструмент) и переведи в 2-3 конкретных шага ДЛЯ НАС, с глаголов. Опирайся на суть поста, а не на общие слова. Про товар/нишу: упоминай ТОЛЬКО там, где это ЕСТЕСТВЕННО обостряет шаг (напр. про категоризацию → «особенно для гигиены полости рта и бьюти»; про правила 18+ → наши спреи 18+; про детскую нишу → детская паста; про сезонность бьюти-процедур → крио). Если применимо ко всем — так и пиши «ко всем нашим товарам», без выдумок. Если пост про ЧИСТУЮ механику МП (финансы, вывод, комиссии, тарифы, логистика, склады) без товарного угла — БЕЗ товаров, просто действие. ЗАПРЕЩЕНО: приплетать список «дентал-салфетки и ирригаторы» ради галочки (это шум) и лить воду «учитывайте при планировании».\nЭТАЛОН СТИЛЯ (повторяй его): пост про техники редактирования карточек WB → apply: «Применимо ко всем нашим товарам на WB: проверить, нет ли карточек в черновике, оптимизировать категоризацию (особенно для гигиены полости рта и бьюти), при необходимости масштабировать редактирование через массовое объединение вариантов»."}
