...
{"keep": true|false,
 ...
 "apply": "КОНКРЕТНОЕ действие для нас, начни с глагола (Пересчитать/Подключить/Отключить/Проверить/Настроить…). Про товар — ГЛАВНОЕ ПРАВИЛО: упоминай конкретный товар ТОЛЬКО если тема поста завязана на ЕГО ОСОБЕННОСТЬ/нишу и это делает совет ОСТРЕЕ по существу (примеры: пост про категорию/правила 18+ → наши спреи 18+; про детскую нишу/мам → детская паста; про отзывы с конкретной претензией → товар, к которому эта претензия относится; про сезонность бьюти-процедур → крио-салфетки/гель). Если пост про ОБЩУЮ механику МП (финансы, вывод денег, комиссии, тарифы, логистика, склады) — НЕ приплетай товары ВООБЩЕ, дай чистое действие. Тупое перечисление «дентал-салфетки и ирригаторы» ради галочки, не связанное с их спецификой — это БЕСПОЛЕЗНЫЙ ШУМ, так писать НЕЛЬЗЯ: лучше совет вообще без товара. НЕ выдумывай товары/форматы/наборы. 1 короткое предложение по сути."}
