...
# С облака (через Scrapfly, ПЛАТНО, соц-ключ 1000/мес) — РОВНО 1 страница/канал,
# чтобы гарантированно влезть в лимит: 28 каналов × 1 = 28 кред/день = 840/мес
# (+dzen/ok ~70 = ~910 из 1000). 1 страница = ~20 свежих постов, вчерашние в
# 09:30 туда попадают. Локально (бесплатно, прямой запрос) листаем глубже.
MAX_PAGES = 1 if SCRAPFLY_KEY else 8
