...
def run(dfrom=None, dto=None):
    ...
    if b_high:
        ...
        parts.append("WB тарифицирует объём с точностью 0,1 л — по завышенному объёму "
                     "логистика и хранение считаются дороже. При расхождении >10% "
                     "добавляется повышающий коэффициент ×5/×10.")
        parts.append("")
    if b_revert:
...
def send_current():
    ...
    lines.append("WB тарифицирует объём с точностью 0,1 л — по завышенному объёму "
                 "логистика и хранение считаются дороже. При расхождении >10% "
                 "добавляется повышающий коэффициент ×5/×10.")
    notify.send("\n".join(lines))
